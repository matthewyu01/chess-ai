...
def convert_board(board):
    ...
    if board.turn == chess.WHITE:
        board_rep2 = np.append(board_rep.reshape(-1), [3]) #first 513 cols are x, then y 
    else: 
...
def get_data():
    ...
    for _ in range(20000):
        ...
        if board.turn == chess.WHITE:
            x = np.append(convert_board(board).reshape(-1), [3, eval_]) #first 513 cols are x, then y 
        else: 
            x = np.append(convert_board(board).reshape(-1), [-3, eval_]) 
        data_list.append(x)

    df = pd.DataFrame(data_list)

    stockfish_data = df.to_csv('Data/stockfish_depth0_g.csv', index = False, header = False) 
